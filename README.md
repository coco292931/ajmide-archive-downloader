...
slots配置是允许交叉的，例如`[{"days": [6], "start": "08:00", "end": "12:00"}, {"days": [6], "start": "08:00", "end": "16:00"}]`和`[{"days": [6], "start": "08:00", "end": "12:00"}, {"days": [6], "start": "18:00", "end": "16:00"}]`都可以被正常识别，但如果当天不同时间段的同名（或同一个节目）同时解析解析成功，那么后面第n个解析成功的文件末尾会加上`_(n-1)`以作区分，形式如`xxx_1.m4a`,`xxx_2.m4a`等。
