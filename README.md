...
由于本项目的最初目的和核心目的是获取hitfm历史节目的下载链接，然而因为阿基米德官方隐藏内容，导致现在不能从hitfm的节目信息页获取到任何有价值的信息，所以本工具暂时不支持直接从节目信息页自动抓取音频文件（对于能打开节目信息页面的，这并不难，具体方法可以看原理的开头部分）。并且由于阿基米德文件保存逻辑，暂时只能通过手动配置文件来实现解析（具体方法和核心代码在原理部分可以找到）。
...
经过各方努力，综合各因素考量，阿基米德 HitFM 存档下载计划正式于2026年4月3日以半成功状态结束。具体报告请见 > [HitFM 阿基米德下载计划 · 最终报告](./finnal_report.md)

---

## 📢 悲报 | 阿基米德 HitFM 历史节目存档或无法找回
...
此前曾乐观估计，阿基米德平台约于 **3月24日前后** 下线 HitFM 相关节目入口时，仅做了类似云听的"隐藏入口"处理，源文件仍可通过直链访问。然而经今日下午实测，情况远比预想严峻。

---

### 技术现状

> 通过对阿基米德的再次抓包，从截获的响应头进行分析

#### CDN 层

- 阿里云 CDN 节点对大量历史节目链接返回 `MISS`（无缓存命中）
- 响应头中可见 `EO-Cache-Status: MISS` 及 `X-Cache: MISS TCP_MISS`，确认缓存已失效
- 目前已知**杭州地区节点**受影响严重，大量链接已无法通过 CDN 获取，估计其他地区的情况也不容乐观
...
目前已确认杭州地区 **2017年6月 ～ 2019年1月** 区间内的节目**几乎全部失效**；广州地区经粗略判断情况几乎相同，为损失最为集中的时间段。

> 杭州地区各时间区间详细链接状态请参见项目 `logs/` 目录。

---

### 形势研判

阿基米德已**官方主动归档**这批文件，这意味着：

- 源站已无法绕过权限直接取回
- 现存的唯一机会窗口是**各地区残余 CDN 缓存**
- CDN 缓存会随时间持续自然衰减，保守估计 **7 天内** 将全部失效
- 届时若无外部介入，这批节目将几乎无法再通过常规手段取回
...
经过一番操作，终于拿到了 c_id 代号表格。

> 在爆破时发现，easy fm 的 c_id 与 hit fm 紧邻。如果你正在进行 easy fm 的搜索工作，你可以优先从 ajmd-res 文件夹内的 phid 和下方的 c_id 入手。

| 节目名称 | c_id | 播出时间 |
| --------- | ------ | --------- |
| the night mix | 459 | 00:00-06:00(1-5),00:00-08:00(6-7);很多年之前 |
...
| hitfm live | 468 | 15:00-16:00(6) |
| 嘻哈公园 | 469 | 16:00-18:00(6) |
...
| hitfm remix | 474 | 20:00-22:00(6;很多年之前) |
...
| CTDM Chart《中国电子音乐巅峰榜》 | 32865 | 20:00-00:00(6-7)、在周日at 40时间段播出(数年前) |
| music flow | 32864 | 00:00-06:00(1-5),00:00-08:00(6-7) |
| Hit FM Dance Carta & Co.电音-卡塔 | 54502 | 20:00-22:00(7) |

~~可惜的是，中国电子音乐巅峰榜（被部分合并）和近几年的music flow没有对应前刀节目，相应的代号经粗略爆破并未获得，希望有uu能获取到。~~感谢吧友[@贴吧用户_JJQMGb1](https://tieba.baidu.com/home/main?id=tb.1.f8f699cc.4B7QffJ7q24pbmMSIVtBOQ%3Ft%3D1738405483&fr=frs)提供线索，现在hitfm的节目单已经基本完善
...
> 该自动转换管线源自 [云听下载器](https://github.com/coco292931/yunting-archive-downloader)。对于阿基米德最高约 60k 码率的音频而言，其实并非必要。
